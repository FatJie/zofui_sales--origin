	...
	<div :class="params.style"  id="page" >
	    ...
	    <div class="main-content" v-if="inited == 2">
	        <div id="main" >
                ...
                <van-swipe :show-indicators="false" :vertical="true" class="top-swiper fade-in slideorder"  :autoplay="4000" :style="{top:params.idxbbtop > 0 ? (dev == 'app' ? params.idxbbtop/2.5 : params.idxbbtop/5) +'px' : '90px'}" v-if="buyed && buyed.length > 0">
                ...
                <div class="item_cell_box index_loc" @click="locCity" :style="{top:params.citytop > 0 ? params.citytop+'px' : '5px'}" v-if="params.iscity == 1">
                    <div class="index_locimg">
                        <img src="../../assets/images/info-shop_icon03.png" >
                    </div>
	        	...
	        	<div id="content" >
                    ...
                    <div class="shop-box" v-if="itemout.name == 'shop'">
                        <div v-for="(item,iidx) in itemout.params.data" :key="iidx" >
                    ...
                    <div class="kill_item" v-if="itemout.name == 'kill'" :style="{padding:itemout.params.paddingv+'px 15px'}">
                        ...
                        <div class="kill_goodslist" v-if="itemout.params && itemout.params.data && itemout.params.data.length > 0">
                            ...
                            <div class="quick-list" v-if="itemout.params.style == 2">
                                <div scrollX scrollWithAnimation="scroll-with-animation">
                                    <div class="list">
                                        <div class="item" v-for="item in itemout.params.data" :key="item.id" :class="itemout.params.goodssize == 1 ? 'quick-listbig' : ''" v-if="itemout.params.data" >
                                            ...
                                            <div class="btm-b">
                                                ...
                                                <div class="btm-info">
                                                    <div class="btm_price fmt-price">
                                                        ...
                                                        <div class="old" v-if="item.show_fx <= 0 && item.fanxm <= 0 && item.userpricejb <= 0 && item.oldprice >0">¥{{item.oldprice}}</div>
                    ...
                    <div class="quest_box" v-if="itemout.name == 'quest'" :style="{padding: itemout.params.paddingv+'px 0.4rem'}">
                        <div class="item_cell_box quest_in">
                            <div class="quest_inl">
                                <img :src="itemout.params.img" :style="{width:itemout.params.width+'px',height:itemout.params.height+'px'}" />
                            </div>
                            <div class="item_cell_flex quest_inr" >
                                <van-swipe :autoplay="4000" vertical :show-indicators="false" loop >
                                    <van-swipe-item v-for="(qitem, innn) in itemout.params.data" :key="innn" @click="link" :data-url="qitem.appurl">
                                        <div class="quest_item">{{qitem.title}}</div>
                                    </van-swipe-item>
                                </van-swipe>
                            </div>
                        </div>
                                ...
                                <div class="btm">
                                    ...
                                    <div class="btm_price fmt-price item_cell_box">
                                        <div class="now color-pri item_cell_flex">价值
                                            ...
                                            <font class="price" v-if="item.oldprice*1 > 0">{{item.oldprice}}</font>
                                        </div>
                                        ...
                                        <div class="btm-b">
                                            <div class="btm-info">
                                                <div class="btm_price fmt-price">
                                                    ...
                                                    <div class="old" v-if="(item.show_fx <= 0 && item.fanxm <= 0  && item.userpricejb <= 0) || itemout.params.style == 0 && item.oldprice >0">¥{{item.oldprice}}</div>
                                                    <div class="btm_fx lbg-pri" v-if="item.show_fx > 0  && item.userpricejb <= 0">赚{{item.show_fx}}</div>
                                                    <div class="btm_fx lbg-pri" v-if="item.show_fx <= 0 && item.fanxm > 0 && item.userpricejb <= 0">返{{item.fanxm}}</div>
                                                    <div class="btm_fx lbg-pri" v-if="item.userpricejb > 0">{{item.userpricejb*100/100}} {{params.umname}}</div>
                                                </div>
        ...
        <shopjoin v-if="params.idxjoin == 0" :isshow="params.idxjoin == 0" :params="params"></shopjoin>
        ...
        <van-popup v-model="isshowtrade" position="right" :overlay="false" style="overflow: hidden;z-index: 105" >
            <div class="trade-box">
                <div class="trade-tit">选择服务区域</div>
                <div class="trade-item" :class="acttrade.id == item.id ? 'color-pri' : ''" v-for="item in trade" @click="changetrade" :data-id="item.id">{{item.name}}</div>
                <div class="trade-item pcity-close" @click="isshowtradefn">关闭</div>
            </div>
        </van-popup>

        <van-popup v-model="isshowcity" position="right" :overlay="false"  style="z-index: 105;" custom-class="trade-out">
            <div class="trade-box">
                
                <div class="i-search">
                    <div class="search citysearch item_cell_box">
                        <div>
                            <img class="iconfonta" src="https://img11.360buyimg.com/ddimg/jfs/t1/38612/25/4015/1361/5cc7b929E39b9b786/cd52b7760b6b8806.png" />
                        </div> 
                        <div class="item_cell_flex">
                            <input type="text" placeholder="请输入搜索内容" value="" @input="searchcity" name="searchcity"  />
                        </div>
                    </div>
                </div>

                <div scroll-y style="height:calc(100vh - 150px);overflow-y: auto;"  >

                    <van-cell :title="item.name" clickable  v-for="(item,index) in scitylist" :key="index" @click="changecity" :data-id="item.pid" >
                    </van-cell>

                    <van-cell :title="item.name" clickable   @click="changecity" :data-id="item.pid" v-for="(item,index) in citylist" :key="index">
                    </van-cell>

                </div>

                <div class="trade-item pcity-close" @click="isshowcityfn">关闭</div>
            </div>
        </van-popup>
...
<script type="text/javascript">
	import { Button,Loading,Swipe,SwipeItem,List,Toast,Dialog,PullRefresh,Image,Lazyload,Popup,Cell,Rate,Icon } from 'vant';
	...
	export default {
        name : 'index',
	  	components: {
            ...
            [Rate.name] : Rate,
            [Icon.name] : Icon,
	    	vipprice,hotsale,shopjoin,good,shoplist,pnav,slider,pimage,ptitle,pfix,pvideo
	  	},
	  	data : function(){
	  		return {
                inited : false,
