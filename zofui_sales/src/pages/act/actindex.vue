        ...
        <div id="content" v-if="inited == 2">
            ...
            <template v-if="datalist.length">
                <van-list v-model="page.doing" :immediate-check="page.initget" :finished="page.isend" loading-text=" " finished-text="" :offset="100" @load="onLoad">
                    <div class="act-list" v-if="!listtype">
                        <div class="item" @click="link" :data-url="item.appurl" v-for="item in datalist" >
                            <div class="top">
                                <div class="image-box __58">
                                    <img :src="item.thumbh">
                                </div>
                                <div class="top_like">
                                    <font class="iconfont theme-color"> </font>{{item.scan}}
                                </div>
                            ...
                            <div class="btm">
                                ...
                                <div class="btm-b">
                                    <div class="btm_price color-pri fmt-price">
                                        <font v-if="item.show_price > 0">¥</font>
                                        <font class="price">{{item.show_price > 0 ? item.show_price : '免费'}}</font>
                                        <font class="old" v-if="item.oldprice*1 > 0">¥{{item.oldprice}}</font>
                                    </div>
                                    <div class="btm_intro">
                                        {{item.endtime}} · 
                                        {{item.saled}}人参与
	...
	export default {
	  	...
	  	data : function(){
	  		return {
                ...
                sortindex : 0,
                showselect : false,
                ordertype : [{name:'距离最近',id:0},{name:'参与最多',id:1},{name:'价格最低',id:2}],
                goodstype : [
                    {name:'进行中的',id:1},
                    {name:'已经结束',id:2},
                    {name:'还未开始',id:3},
