<template>
    <div :class="params.style">
        ...
        <div class="app-support copybox" v-if="copy.iscopy == 1 && isshowcopy && curPage != '/login' && curPage != 'kefuindex'">
            <div class="app-supportin" @click="bindtap($event,'callphone')" :data-tel="copy.copytel">
                <img class="pp-supportimg-top" v-if="copy.copytop" :src="copy.copytop">
                <div class="app-supportin-bot">
                    <div class="pp-supportimg pp-item" v-if="copy.copyimg">
                        <img :src="copy.copyimg" >
                    </div>
                    <div class="pp-supporttip pp-item" v-if="copy.copyfa || copy.copyfb">
                        <div v-if="copy.copyfa" >{{copy.copyfa}}</div>
                        <div v-if="copy.copyfb" >{{copy.copyfb}}</div>
                    </div>
                    <div class="pp-supportbtn pp-item" v-if="copy.copybtn">
                        <div class="button lbg-pri">{{copy.copybtn ? copy.copybtn : '查看'}}</div>
                    </div>
                </div>
            </div>
        </div>

        <div v-if="Menu && !Menu.data && isshow">
            <div class="fix-box" >
                <div class="app-tabbar">
                    <div @click="link" :data-url="item.appurl" class="item" :class="{'active':curPage == item.appurl}"  v-for="item in Menu" >
                        <img :src="curPage == item.appurl ? item.selectedIconPath : item.iconPath" />
                        <div>{{item.title}}</div>
                    </div>
                </div>
        ...
        <div  v-else-if="isshow">
            <div class="fix-box" >
                <div class="app-tabbar">
                    <div @click="link" :data-url="index == 0 ? '/' : item.appurl" class="item" :class="{'active':curPage == item.appurl || (index == 0 && curPage == '/')}" v-for="(item,index) in Menu.data" >
                        <img :src="curPage == item.appurl || (index == 0 && curPage == '/') ? item.actimg : item.img " />
                        <div :style="{color:curPage == item.appurl || (index == 0 && curPage == '/') ? Menu.actcolor : Menu.color}">{{item.name}}</div>
                    </div>
                </div>
                <div class="is-ipxa" v-if="isIPX"></div>
            </div>
	...
	export default {
        ...
        methods : {
            ...
            getPath(){
                ...
                var adminlist = ['adminindex','adminmoney','adminorder','adminorderinfo','adminset','adminsignin','admingoods','admindraw','adminbadmin','adminbhelplist','adminjifenadd','adminjifenlog','adminjifengive','cardredbag','admincardlog'];
            ...
            hideBar(){
                var hidelist = ['login','goodinfo','shopinfo','cardmyinfo','jiziinfo','adminbindadmin','adminhexiao','adminhexiaocard','commentinfo','orderinfo','mymoney','jifengoods','signinfo','zhuanpan','redbaginfo','commentorder','orderaddaddress','orderconfirm','orderaddress','groupinfo','agentjoin','adminpay','fenxiaoinvite_shop','buycard','adminsignin','cardinfo','adminpub','agentinvite','adminbaddcard','adminbcard','adminset','fenxiaojoinPartner','kefuindex','vedioindex','art','yhminfo','questinfo','goodsshare','addshopmoney'];
            ...
            registShare(){
                ...
                if (ua.match(/MicroMessenger/i) == "micromessenger") {
                    self.$util.http('myinit','GET',{op:'registShare',jsurl:location.href},function(res){
                        if( res.data && !res.errno ){
                            if( res.data.jssdk ){
                                wx.config({
                                    debug: false,
                                    appId: res.data.jssdk.appId,
                                    timestamp: res.data.jssdk.timestamp,
                                    nonceStr: res.data.jssdk.nonceStr,
                                    signature: res.data.jssdk.signature,
                                    jsApiList: [
